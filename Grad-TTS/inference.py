...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--speaker_ids', type=int, required=False, nargs="+", default=None, help='speaker ids for multispeaker model')
    args = parser.parse_args()

    for speaker_id in args.speaker_ids:
        save_folder = f"out/{args.checkpoint.split('/')[-1].split('.')[0]}/s{speaker_id}_{args.timesteps}i/"
        ...
        with torch.no_grad():
            for i, text in enumerate(texts):
                ...
                write(os.path.join(save_folder,f"sample_{i}.wav"), 22050, audio)
